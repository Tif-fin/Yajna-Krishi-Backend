...
def lcc_001(request):
    ...
    if request.method == 'GET':
        ...
        if version:
            ...
            latest = max[-1]
            if latest == version:
                return JsonResponse({'status': 'App is up to date', 'latest': latest})
            elif version not in max:
                return JsonResponse({'error': 'Invalid version'})
            elif latest != version:
                return JsonResponse({'status': 'Update available', 'latest': latest, 'url':'https://'+request.headers.get('X-Forwarded-Host')+'/lcc/download?filename='+latest})
        else:
